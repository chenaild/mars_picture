		...
		<view class="cu-card dynamic no-card">
			<view class="cu-item shadow">
				<view class="grid flex-sub padding-lr col-2 grid-square">
					<view v-for="(item, index) in photos" :key="index" @tap="info" class="bg-img" :data-src="item.pic"
						:style="'background-image:url(' + item.pic + ');'"></view>
				</view>
			</view>
		</view>
	</view>
						...
						for (;;) switch (t.prev = t.next) {
							case 0:
								return t.next = 2, new Promise(function(t, e) {
									a.request(
										"https://spare.maibaapp.com/bbs/newUgc/category/list/0"
										).then(function(n) {
					...
					return e.default.wrap(function(t) {
						for (;;) switch (t.prev = t.next) {
							case 0:
								return t.next = 2, new Promise(function(t, e) {
									a.request(
										"https://spare.maibaapp.com/bbs/newUgc/category/info/0/" +
										n + "/" + r + "/" + (r + 17)).then(
										function(n) {
											200 == n.statusCode && "request:ok" ==
												n.errMsg && 0 == n.data.code &&
