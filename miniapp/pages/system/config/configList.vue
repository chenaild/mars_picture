<template>
	<view v-if="vuex_user.system == 1">
		<view class="u-p-l-20 u-p-r-20">
			<view class="tab u-m-t-20" v-for="(item, index) in configList" :key="index"
				...
				<view class="u-font-12 u-tips-color u-p-l-30 u-p-r-30 u-p-t-15 u-p-b-15 u-flex u-row-between">
					...
					<view class="">
						<view class="" v-if="item.val === true || item.val === false">
							<u-switch v-model="item.val" size="28" active-color="#f1c929" inactive-color="#F8F8F8"
								@change="switchChange(item._id, index)"></u-switch>
						</view>
						<view class="u-flex u-row-right" v-else @click="clickConfig(item._id, index)">
							<view class="u-p-r-10">编辑</view>
		...
		<u-modal v-model="editShow" title="配置" :show-cancel-button="true" confirm-text="保存" confirm-color="#fcc600"
			content="content" :async-close="true" @confirm="editChange" negative-top="280">
			<view class="u-p-40">
				<view class="">
					<u-input v-model="config.name" :border="true" border-color="#F7F7F7" :clearable="false" :height="80"
						disabled placeholder="填写配置的说明" />
				</view>
				<view class="u-p-t-15">
					<u-input v-model="config.val" :border="true" border-color="#F7F7F7" :clearable="false" :height="80"
						placeholder="填写配置的值" />
				</view>
			</view>
		</u-modal>
