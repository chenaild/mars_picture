	...
	<view>
		...
		<view class="">
			<view class="u-flex u-row-left u-flex-wrap u-p-l-40 u-p-t-20">
				<view class="tab u-skeleton-fillet" @longpress="moreClick(item._id, true)" @click="detail(item._id, item.mode)" v-for="(item, index) in coverList" :key="index">
...
<script>
	...
	export default {
		...
		methods: {
			...
			detail(id, mode){
				uni.navigateTo({
					url: '/pages/detail/detail?id='+id+'&index=0'+'&mode='+mode 
				})
			},
		},
		onReachBottom(){
